describe "migration spec" do
  ...
  describe "Migration" do
    it "is possible to migrate the breadcrumbs helper" do
      template = <<-HBS
        {{foobar}}

        {{section.name}}

        {{#if section.internal}} Internal {{/if}}
      ...
      ast = Curlybars.ast(template, 'section_page.hbs', run_processors: false)

      byebug

      finder = AstPathFinder.new(ast)

      matches = finder.find("article.title")
...
class AstPathFinder
  ...
  def resolve_path(path_segments, context_stack)
    # Handle ../ prefix which navigates up the context
    # The path might be like ["../title"] or ["../../title"] etc.
    full_path = path_segments.join('.')
    stack = context_stack.dup

    # Count and remove ../ prefixes
    while full_path.start_with?('../')
      full_path = full_path[3..-1] # Remove '../'
      stack.pop unless stack.empty?
    end

    # Split the remaining path and combine with context
